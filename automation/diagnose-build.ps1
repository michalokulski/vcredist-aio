...
if (-not $ps2exeModule) {
  $issues += "Install PS2EXE: Install-Module ps2exe -Scope CurrentUser -Force"
}
...
if (Test-Path "packages.json") {
  ...
  if ($emptyVersions.Count -gt 0) {
    $issues += "Populate package versions: pwsh automation/update-check.ps1 -PackagesFile packages.json"
  }
}
...
if ($issues.Count -eq 0) {
  ...
  Write-Host "    pwsh automation/build-ps2exe.ps1 -VerboseBuild" -ForegroundColor White
} else {
